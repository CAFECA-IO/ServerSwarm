...
(git clone https://github.com/CAFECA-IO/AICH.git /aich/app && git checkout $TARGET_BRANCH) || (cd /aich/app && git pull origin $TARGET_BRANCH)
# Info: (20240821 - Jacky) 進入 /isunfa/app 目錄並安裝依賴（假設是 Node.js 應用）
cd /aich/app && npm install
...
PORT=${PORT} pm2-runtime start npm --name "aich" -- run start
